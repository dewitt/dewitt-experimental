...
sub ACTION_build
{
  ...
  $VARS{ 'ProjectPath' } = $self->_get_project_path( );
...
sub _get_project_path
{
  my ( $self ) = @_;

  my $project_path = $self->{ 'properties' }->{ 'project_path' } ||
    $self->{ 'properties' }->{ 'project_name' } or
      die( "Couldn't get project path" );
