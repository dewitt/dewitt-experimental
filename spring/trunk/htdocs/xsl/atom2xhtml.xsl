...
<xsl:stylesheet version="1.0"
 ...
 >
	...
	<xsl:template match="atom:feed | rss/channel | rdf:RDF">
		...
		<html xml:lang="{$language}" lang="{$language}">
			...
			<head>
				...
				<link rel="stylesheet" href="{$project-url}/static/css/atom.css" type="text/css" title="default" media="screen" />
				<!-- rel links -->
			...
			<body>
				...
				<xsl:choose>
					<xsl:when test="$items">
						...
						<!-- hack for xslt processors that do not respect disable-output-escaping="yes" --><xsl:if test="string-length($use-doe-hack)"><span id="encoding-test" style="display:none"><xsl:text disable-output-escaping="yes">&amp;amp;</xsl:text></span><script type="text/javascript" src="{$project-url}/static/js/atom.js"></script></xsl:if>
						<!-- result navigation -->
						<p id="nav">
	...
	<xsl:template name="showtext">
		<xsl:param name="node" />
		<xsl:choose>
			<xsl:when test="name($node)='description' or name($node)='title' or $node/@type='html'"><div class="x-escape"><xsl:value-of select="$node" disable-output-escaping="yes" /></div></xsl:when>
