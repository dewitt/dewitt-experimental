...
sub _get_dispatcher
{
  ...
  my $best_dispatcher_length = -1;
  my $is_ambiguous = 0;

  my $configuration = $self->_get_configuration( ) or
    throw Error::Simple( "Couldn't get configuration" );

  exists $configuration->{ 'dispatchers' } or
    throw Error::Simple( "No dispatchers in configuration" );
...
sub _match_dispatcher
{
  ...
  my @dispatcher_elements = $self->_get_elements( $dispatcher_path );

  my @path_elements = $self->_get_elements( $path );
