...
sub get_api_html
{
  ...
  foreach my $dispatcher ( @{$self->_get_dispatchers( )} )
  {
    push( @dispatchers_data, { 'path' => $dispatcher->{ 'path' },
                               ...
                               'link' => $self->_get_link( $request, $dispatcher ) } );
   }
...
sub _get_link
{
  my ( $self, $request, $dispatcher ) = @_;

  my $site_info = $self->get_site_info( $request ) or
    throw Error::Simple( "Couldn't get site info" );

  my $link = $site_info->{ 'project_url' } . $dispatcher->{ 'path' };

  $link =~ s/\[\w+\?\]//g;
  $link =~ s/\[searchIndex\]/Books/g;
  $link =~ s/\[searchTerms\]/Harry+Potter/g;
  $link =~ s/\[startPage\]/1/g;
