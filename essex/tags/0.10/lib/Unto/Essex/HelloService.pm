...
sub say_something_optional
{
  my ( $self, $request, $params ) = @_;
  my $salutation = $params->{ 'salutation' } or
    throw Error::Simple( "Couldn't find salutation" );
  my $optional = $params->{ 'optional' } || '';
  return "$salutation-$optional";
}
...
sub lookup_and_release
{
  my ( $self ) = @_;
  my $hello = $self->_lookup( 'hello' ) or
    throw Error::Simple( "Couldn't lookup hello" );
  $hello->say_hello( ) eq 'Hello World!' or
    throw Error::Simple( "Couldn't say hello" );
  $self->_release( $hello );
  return 1;
}
